...
export function Footer() {
  ...
  const c = useSection("contact", CONTACT_DEFAULTS);
  const f = useSection("footer", FOOTER_DEFAULTS);
  return (
    <footer className="relative overflow-hidden border-t border-tvk-gold/30 bg-gradient-to-b from-tvk-black via-[#0a0405] to-black">
      ...
      <div className="relative mx-auto max-w-7xl px-5 py-12 sm:px-8 sm:py-14">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
          <div>
            ...
            <div className="mt-4 flex items-center gap-3">
              <a href={c.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br from-yellow-500 via-pink-500 to-purple-600 text-white transition hover:scale-110">
                <Instagram className="h-5 w-5" />
              </a>
              <a href={c.whatsapp} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="grid h-10 w-10 place-items-center rounded-full bg-green-600 text-white transition hover:scale-110">
          ...
          <div>
            <div className="mb-3 font-display text-sm uppercase tracking-[0.3em] text-tvk-red">Explore</div>
          ...
          <div>
            <div className="mb-3 font-display text-sm uppercase tracking-[0.3em] text-tvk-red">Reach Us</div>
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex items-start gap-2"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-tvk-gold" /><span>{c.address}</span></li>
              <li className="flex items-start gap-2"><Mail className="mt-0.5 h-4 w-4 shrink-0 text-tvk-gold" /><span>{c.email}</span></li>
              <li className="flex items-start gap-2">
                <MessageCircle className="mt-0.5 h-4 w-4 shrink-0 text-tvk-gold" />
                <a href={c.whatsapp} target="_blank" rel="noopener noreferrer" className="transition hover:text-tvk-gold">Join WhatsApp Community</a>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-tvk-gold/20 pt-6 text-center md:flex-row md:text-left">
          <p className="text-xs text-muted-foreground">© {year} Tamilaga Vettri Kazhagam — Marayoor Unit. All rights reserved.</p>
          <p className="font-display text-xs uppercase tracking-[0.35em] text-tvk-gold">{f.tagline}</p>
          <p className="text-xs text-muted-foreground">
            <a href={f.credit_url} target="_blank" rel="noopener noreferrer" className="font-medium text-tvk-gold transition hover:text-white hover:underline">
              {f.credit_text}
            </a>
          </p>
        </div>
